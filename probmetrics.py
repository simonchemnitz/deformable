...
def mu_t(p1: np.ndarray, p2: np.ndarray, t: float) -> float:
    """
...
def rho(p1: np.ndarray, p2: np.ndarray) -> float:
    """
    ...
    µ(0.5)
    """
    return mu_t(p1, p2, 0.5)


def bhattachayya_distance(p1: np.ndarray, p2: np.ndarray) -> float:
    """
    ...
    B: float
        ...
        between p1 and p2
    """
    return -np.log(rho(p1, p2))


def chernoff_information(p1: np.ndarray, p2: np.ndarray, n_points=500) -> float:
    """
    ...
    vals = np.array(list(map(lambda t: mu_t(p1, p2, t), ts)))
